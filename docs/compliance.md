...
> 本文件为 `media-content-insight` 项目的硬性合规红线,与 `requirements.md` 中需求 19、20 一致,并与 `design.md` 第 5 节「合规与风控边界」对齐。
>
...
- 使用者应自行遵守目标平台条款及当地法律法规;不当使用所产生的全部后果由使用者承担,与本项目作者无关。
...
- 系统 **仅** 实现「关键词搜索 → 采集内容 + 评论 → 本地分析」一条数据流。
...
后端 **不存在** 任何向目标平台发起以下写操作的代码路径,前端 **不暴露** 任何对应入口、按钮、菜单项、表单或 API 调用:

| 编号 | 禁止类别 | 具体含义 | 实现侧约束 |
|------|----------|----------|------------|
| 19.2 | 自动评论 | POST 评论、回复评论 | `Crawler_Service` 调用 MediaCrawler 时 `--type` 取值 **仅限 `search`** |
| 19.3 | 自动点赞 | 点赞 / 取消点赞内容或评论 | 不向 MediaCrawler 传递任何点赞相关参数 |
| 19.4 | 自动收藏 | 收藏 / 取消收藏内容 | 不向 MediaCrawler 传递任何收藏相关参数 |
| 19.5 | 自动私信 | 发起 / 回复私信 | 不向 MediaCrawler 传递任何私信相关参数 |
| 19.6 | 自动关注 | 关注 / 取关用户 | 不向 MediaCrawler 传递任何关注相关参数 |
| 19.7 | 自动发布 | 发布 / 编辑 / 删除内容 | 后端无任何写内容代码路径 |
...
| 约束项 | 取值 | 强制点 |
|--------|------|--------|
| 单次采集内容上限 | `max_notes ∈ [1, 20]` | API 层校验 `OVER_LIMIT`;写入时 `slice(0, max_notes)` |
...
- [ ] `grep` 全仓未发现向目标平台发起评论 / 点赞 / 收藏 / 私信 / 关注 / 发布的代码路径
